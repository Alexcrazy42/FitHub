// Взято из https://github.com/alex1ozr/DockerTests/blob/main/tests/Api.IntegrationTests/Infrastructure/TestServerMessageFilter.cs под MIT
...
internal sealed class TestServerMessageFilter : IHttpMessageHandlerBuilderFilter
{
    ...
    private sealed class TestServerHandler : DelegatingHandler
    {
        ...
        {
            serverAuthority = GetAuthority(server.BaseAddress).Required();
            ...
            CancellationToken cancellationToken)
        {
            return IsRequestToTestServer(request)
                ...
                ? serverHandler.ExecuteAsync(request, cancellationToken)
        ...
        {
            return GetAuthority(request.RequestUri) == serverAuthority;
        }
