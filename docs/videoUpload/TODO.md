...
## 10. `QualityLabel` в контракте — число вместо enum

`VideoResolutionResponse.QualityLabel` и `VideoResolutionUrlResponse.QualityLabel` возвращают `int`.
Лучше использовать строковый enum `VideoQuality` (`"Q360P"`, `"Q720P"`, `"Q1080P"`), который уже есть в домене,
либо явно задокументировать значения.

**Файлы:** `Contracts/V1/Videos/VideoResponses.cs:25`

---

## 11. RabbitMQ producer/consumer — независимый старт
