...
1. Статическая
   ConnectionStrings, Kafka bootstraps, urls
   Частота изменений: при создании окружения
   Провайдеры: appsettings, k8s secrets, vault
2. Операционная
   Фича-флаги, таймауты, лимиты, параметры ретраев, процент канареечного трафика
   Частота изменений: несколько раз в день/неделю
   Провайдеры: Consul, Volume-mounted ConfigMap + IOptionsMonitor
3. Реалтайм
   Данные, которые относятся к конкретному запросу, сессии
   Курс валют для транзакции конкретной, access tokens, координаты, корзина пользователя
   Частота изменений: сотни-тысячи раз в секунду
   Провайдеры: redis, memory cache, БД (enabled для сущностности, например)
